...
def _url(suffix):
    return f'https://www.realestateinrincon.com/{suffix}'


def get_j_and_m_condos():
...
def _get_rincon_condo_urls(page):
    ...
    for anchor in page('a'):
        if 'Suite' in str(anchor) and 'Residential' in str(anchor):
            condo_urls.append(anchor.get('href', 'no url'))
    return condo_urls
